...
(function (global) {
  ...
  AudioManager.prototype.unlock = function () {
    ...
    if (ctx.state === 'suspended' && ctx.resume) {
      ctx.resume().catch(function () { /* ignored */ });
    }
    this.unlocked = true;
  };
  ...
  AudioManager.prototype.applySettings = function (settings) {
    ...
    this.settings.volume = Math.min(1, Math.max(0, Number(settings.volume) || 0));
      ...
      global.fetch(url)
        ...
        .then(function (data) {
          return new Promise(function (resolve, reject) {
            ...
            var out = self.ctx.decodeAudioData(data, resolve, reject);
            if (out && typeof out.then === 'function') out.then(resolve, reject);
          });
        })
        .then(function (buffer) { self.buffers[name] = buffer; })
        .catch(function () { /* no sample: the synth voice is used instead */ });
    });
  };
  ...
  AudioManager.prototype.play = function (name) {
    ...
    var ctx = this.ensureContext();
    if (!ctx) return;
    if (ctx.state === 'suspended') this.unlock();
    ...
    try {
      ...
      this.synth(name);
    } catch (err) { /* never break gameplay for a sound */ }
  };
  ...
  AudioManager.prototype.synth = function (name) {
    switch (name) {
      case 'eat':
        this.tone({ type: 'triangle', from: 440, to: 880, duration: 0.11, gain: 0.25 });
        break;
      case 'bonus':
        this.tone({ type: 'triangle', from: 660, to: 990, duration: 0.10, gain: 0.24 });
        this.tone({ type: 'sine', from: 990, to: 1320, duration: 0.14, gain: 0.18, delay: 0.08 });
        break;
      case 'click':
        this.tone({ type: 'square', from: 320, to: 260, duration: 0.05, gain: 0.10 });
        break;
      case 'levelup':
        this.tone({ type: 'triangle', from: 392, to: 392, duration: 0.14, gain: 0.2 });
        this.tone({ type: 'triangle', from: 523, to: 523, duration: 0.14, gain: 0.2, delay: 0.12 });
        this.tone({ type: 'triangle', from: 659, to: 784, duration: 0.3, gain: 0.22, delay: 0.24 });
        break;
      case 'highscore':
        this.tone({ type: 'sine', from: 523, to: 523, duration: 0.12, gain: 0.2 });
        this.tone({ type: 'sine', from: 659, to: 659, duration: 0.12, gain: 0.2, delay: 0.1 });
        this.tone({ type: 'sine', from: 784, to: 784, duration: 0.12, gain: 0.2, delay: 0.2 });
        this.tone({ type: 'sine', from: 1046, to: 1046, duration: 0.45, gain: 0.24, delay: 0.3 });
        break;
      case 'gameover':
        this.tone({ type: 'sawtooth', from: 330, to: 110, duration: 0.55, gain: 0.18 });
        this.tone({ type: 'sine', from: 220, to: 70, duration: 0.7, gain: 0.16, delay: 0.1 });
        break;
      case 'pause':
        this.tone({ type: 'sine', from: 520, to: 380, duration: 0.12, gain: 0.14 });
        break;
      case 'resume':
        this.tone({ type: 'sine', from: 380, to: 560, duration: 0.12, gain: 0.14 });
        break;
      default:
        this.tone({ type: 'sine', from: 440, to: 440, duration: 0.08, gain: 0.12 });
    }
  };
  ...
  AudioManager.prototype.startMusic = function () {
    ...
    this.musicTimer = global.setInterval(function () { self.scheduleMusic(); }, 60);
  };
  ...
  AudioManager.prototype.scheduleMusic = function () {
    if (!this.ctx || !this.settings.music) { this.stopMusic(); return; }
    var stepDuration = 0.26;           // ~115 bpm eighth notes
    var horizon = this.ctx.currentTime + 0.4;
    var pattern = [0, 2, 4, 2, 5, 4, 2, 0];

    while (this.nextNoteAt < horizon) {
      var step = this.musicStep % 8;
      var delay = this.nextNoteAt - this.ctx.currentTime;
      if (delay >= 0) {
        if (step % 4 === 0) {
          this.tone({ type: 'sine', from: 110, to: 110, duration: 0.3, gain: 0.16, delay: delay, bus: this.musicGain });
        }
        var note = SCALE[pattern[step]] * (this.musicStep % 16 >= 8 ? 1.5 : 1);
        this.tone({ type: 'triangle', from: note, to: note, duration: 0.22, gain: 0.07, delay: delay, bus: this.musicGain });
      }
